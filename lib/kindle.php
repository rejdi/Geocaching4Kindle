...
function createKindle($session_id, $intermediate, $process, $settings) {
	...
	if ($settings['withImages'] && $settings['map']) {
		...
		foreach ($caches as $cache) {
			...
			fetchMap($session_id, 'http://localhost/gc/staticmaplite/staticmap.php?zoom=15&size=640x480&maptype=hike&markers='.$cache->map->lat.','.$cache->map->lon.',icon:http://www.geocaching.com/images/WptTypes/pins/'.$cache->map->wptid.'.png&sensor=false&center='.$cache->map->lat.','.$cache->map->lon, 'result/'.$session_id.'/map_'.$cache->code.'.png');
		}
	}

...
Class staticMapLite {
	...
	protected $tileSrcUrl = array(	'mapnikbw' => 'http://a.tiles.wmflabs.org/bw-mapnik/{Z}/{X}/{Y}.png',
	...
	public function parseParams(){
		...
		if($_GET['markers']){
			...
			foreach($markers as $marker){
					...
					$pos = stripos($markerImage, 'icon:');
					if ($pos === FALSE) {
						$markerImage = basename($markerImage);
					} else {
						$markerImage = substr($markerImage, $pos + 5);
					}
	...
	public function placeMarkers(){
		foreach($this->markers as $marker){
			...
			if ($markerImage) {
				if (stripos($markerImage, 'http:') !== FALSE) {
					$markerFilename = $markerImage;
				} else if (file_exists($this->markerBaseDir.'/lightblue'.$markerIndex.'.png')) {
					$markerFilename = $this->markerBaseDir.'/'.$markerImage.".png";
				} else if (file_exists($this->markerBaseDir.'/'.$markerImage.".png")) {
					$markerFilename = $this->markerBaseDir.'/lightblue'.$markerIndex.'.png';
				} else {
					$markerFilename = $this->markerBaseDir.'/lightblue1.png';
				}
			} else {
				$markerFilename = $this->markerBaseDir.'/lightblue1.png';
			}
